...
def assemble_tree(source_ref: str, denylist: list[str], out_dir: Path) -> list[Path]:
    ...
    files = [rel for rel in git_tracked_files(source_ref) if not is_denylisted(rel, denylist)]

    # One long-lived `git cat-file --batch` process instead of a `git show`
    # subprocess per file - confirmed live (2026-09-22) that the per-file
    # loop was the dominant cost of the "Publish to Github" pipeline's
    # sanitizer step (~65ms/file locally, ~7 minutes total on CI for 670
    # files, almost all of it process-spawn overhead). Batches request/
    # response over one pipe instead: ~3s locally for the same 670 files.
    #
    # `git archive <ref> | tar -x` looked like an even simpler bulk
    # alternative and was tested first, but confirmed live to silently
    # return wrong blob content for some paths on this machine (git
    # 2.53.0.windows.2) - headers/filenames correct, content from an
    # unrelated blob, `git fsck` shows no actual repo corruption so it's a
    # bug/quirk in `archive` itself, not this repo's data. Do not switch to
    # `git archive` for this without re-verifying byte-for-byte against
    # `git show` on whatever git version actually runs it.
    ...
    def _write_requests() -> None:
        assert proc.stdin is not None
        for rel in files:
            proc.stdin.write(f"{source_ref}:{rel}\n".encode())
        proc.stdin.close()

    writer = threading.Thread(target=_write_requests)
    writer.start()
    ...
    for rel in files:
        ...
        if header.endswith("missing"):
            raise RuntimeError(f"git cat-file --batch: {rel} missing at {source_ref}")
        _sha, _type, size_str = header.split()
        content = out.read(int(size_str))
        out.read(1)  # trailing newline after the object content
        dest = out_dir / rel
...
def build_exact_replacer(m: dict):
    ...
    for i, (section, key, value) in enumerate(entries):
        ...
        prefix += r"(?<!== ')(?<!==\")"
        if section == "names" and key in ("Gavinwoolley", "Gavin"):
            # This repo's own public GitHub mirror IS "gavinwoolley/homeassistant"
            # (and its published image is "ghcr.io/gavinwoolley/homeassistant-demo")
            # - both intentionally public already, since the mirror repo's own
            # name/package path can't be secret. Redacting the owner segment out
            # of a URL that points back at the mirror itself breaks the URL
            # outright rather than protecting anything. Confirmed live
            # (2026-09-25): this corrupted demo/configuration.yaml's 3 new
            # GitHub API/raw-content sensor resource: URLs into a nonexistent
            # owner, 404ing every request in the published demo image.
            #
            # Both "Gavinwoolley" (the compound, normally consumed whole -
            # see that entry's own comment about avoiding a dangling
            # "...woolley" fragment) AND the shorter "Gavin" need this same
            # guard: blocking only the compound lets the alternation fall
            # through to the bare "Gavin" alternative instead, which has no
            # trailing boundary and cheerfully matches just the "gavin"
            # prefix of the URL's "gavinwoolley" - confirmed live
            # (2026-09-25, same incident): that produced
            # ".../repos/homer_simpsonwoolley/..." - worse than the
            # original bug, since it now leaks the real "woolley" surname
            # fragment into the public mirror instead of just breaking a URL.
            prefix += r"(?<!repos/)(?<!githubusercontent\.com/)(?<!github\.com/)(?<!ghcr\.io/)"
        gname = f"g{i}"
...
def exact_pass(files: list[Path], replacer) -> dict[str, int]:
    ...
    for path in files:
        ...
        if new_text != text:
            # newline="" - text-mode write's default translates every \n to
            # os.linesep, silently turning LF blobs into CRLF on Windows.
            # Confirmed live (2026-09-22): this was the sole cause of Gate D
            # (yamllint) failing on every local Windows run - never happens
            # on the real pipeline (Linux agent, os.linesep is already "\n"),
            # but blocks genuine local testing/dry-runs on this platform.
            #
            # Path.write_text() only gained a newline= parameter in Python
            # 3.10 - the real pipeline runs python3.9 (azure-pipelines-
            # publish.yml, pinned since the agent's own python3 is 3.6).
            # Confirmed live (2026-09-23): passing newline= to write_text()
            # on 3.9 is a hard TypeError, breaking the real pipeline outright
            # - open()'s newline= support predates 3.9 by over a decade, so
            # use that instead everywhere this matters.
...
def hash_sub(value: str, kind: str, salt: str) -> str:
    ...
    if kind == "ip_entity_id":
        last = int(digest[:8], 16) % 254 + 1
        return f"10_0_0_{last}"
    if kind == "subnet":
        return "10.0.0.0/24"
    if kind == "digits21":
    ...
    for path in files:
        ...
        for name, pattern, kind in compiled_hash:
            ...
            if n:
                hits[name] = hits.get(name, 0) + n
        if text != original:
            # open(), not write_text() - see exact_pass's comment above (3.9
            # compat: write_text() only gained newline= in 3.10).
            with open(path, "w", encoding="utf-8", newline="") as f:
                f.write(text)
    return hits
...
# Stamps the actual demo image version into the Codespaces devcontainer
# templates, instead of the `:latest` tag demo-image.yml (GitHub Actions)
# also always pushes. Confirmed live (2026-09-23): a user's freshly-created
# Codespace showed entity_ids that have never existed anywhere in this
# repo's git history - the `:latest` tag alone gives no way to tell whether
# a Codespace (or Docker itself) is actually running a stale cached layer
# versus a genuinely fresh pull, since the tag string never changes.
#
# Requested directly (2026-09-25): use the actual Azure DevOps pipeline
# build number (azure-pipelines-publish.yml's `name: $(Major).$(Minor).$(Patch)`
# - a real, incrementing semantic version like "1.0.42"), not a git hash -
# passed in as --pipeline-version by the Publish stage. Local/dry-run
# invocations (including demo/tools/build_demo_dashboards.py's own nested
# `sanitize.py --dry-run` call, which has no pipeline context at all) have
# nothing to pass there, so main() falls back to this private repo's own
# source commit's short sha in that case - still unique, just not a real
# version number.
#
# Whichever value it resolves to is embedded in two places that have to
# agree without talking to each other directly: this function stamps it
# into devcontainer.json/docker-compose.codespaces.yml here, and
# demo-image.yml parses the SAME value back out of the mirror commit's own
# message ("Sanitized public mirror - version <x> (source <short_sha>)",
# git_publish's commit message below) to tag the image it builds. Neither
# side needs to know anything about the other beyond that shared commit
# message format.
DEMO_IMAGE_VERSION_PLACEHOLDER = "__DEMO_IMAGE_VERSION__"


def apply_demo_image_version(out_dir: Path, image_version: str) -> int:
    # Scoped to .devcontainer/ only - NOT out_dir.rglob("*"). The
    # placeholder text also appears, deliberately, in two other places that
    # must NOT be touched: publish/ itself ships as part of the public
    # mirror for transparency (same reasoning main() already applies when
    # excluding "publish/" from the redaction passes), so both this
    # function's own source comment AND publish/templates/.devcontainer/'s
    # template-source copy would otherwise get their literal
    # "__DEMO_IMAGE_VERSION__" documentation text silently replaced with
    # this one run's resolved version - confirmed live (2026-09-23) that an
    # unscoped rglob did exactly that, plus corrupted
    # .github/workflows/demo-image.yml's own comment explaining the
    # mechanism. Only .devcontainer/ (the actual deployed Codespaces
    # config, copied to that path by apply_templates() above) should ever
    # have the placeholder resolved.
    hits = 0
    for path in (out_dir / ".devcontainer").rglob("*"):
        if not path.is_file() or not _is_scannable_text(path):
            continue
        text = path.read_text(encoding="utf-8")
        if DEMO_IMAGE_VERSION_PLACEHOLDER not in text:
            continue
        with open(path, "w", encoding="utf-8", newline="") as f:
            f.write(text.replace(DEMO_IMAGE_VERSION_PLACEHOLDER, image_version))
        hits += text.count(DEMO_IMAGE_VERSION_PLACEHOLDER)
    return hits
...
def write_ci_safe_secrets(out_dir: Path) -> None:
    ...
    if not target.exists():
        return
    # open(), not write_text() - see exact_pass's comment for why (3.9
    # compat), and newline="" for the same CRLF-on-Windows reason as
    # everywhere else this matters.
...
def _leftover_pattern(needle: str, style: str) -> re.Pattern:
    ...
    prefix = r"(?<![A-Za-z0-9])"
    if needle in ("Gavinwoolley", "Gavin"):
        # Mirrors the matching exemption in build_exact_replacer() (both
        # "Gavinwoolley" and the shorter "Gavin" need it, same reasoning -
        # see that function's comment) - the redaction pass deliberately
        # leaves this name intact when it's the owner segment of a URL
        # pointing back at this repo's own public GitHub mirror/ghcr image,
        # so the leftover scan must not flag that same, intentionally-
        # preserved text as a leak.
        prefix += r"(?<!repos/)(?<!githubusercontent\.com/)(?<!github\.com/)(?<!ghcr\.io/)"
    if style == "loose":
        suffix = ""
    elif style == "coord":
...
def git_publish(out_dir: Path, github_url: str, pat: str, source_sha: str, image_version: str) -> None:
    def run(*args):
        subprocess.run(args, cwd=out_dir, check=True)

    run("git", "init", "-q")
    run("git", "checkout", "-q", "-b", "main")
    run("git", "-c", "user.name=Gavin Woolley", "-c", "user.email=gavinwoolley@hotmail.com",
        "add", "-A")
    run("git", "-c", "user.name=Gavin Woolley", "-c", "user.email=gavinwoolley@hotmail.com",
        "commit", "-q", "-m",
        f"Sanitized public mirror - version {image_version} (source {source_sha[:12]})")
...
def main() -> int:
    ...
    ap.add_argument("--pipeline-version", default=None,
                     help="Real semantic version to tag the demo images with - Azure "
                          "DevOps's $(Build.BuildNumber), passed in by azure-pipelines-"
                          "publish.yml's Publish stage. Falls back to the source commit's "
                          "short sha when omitted (local/dry-run use - no pipeline "
                          "context to draw a real version from).")
    args = ap.parse_args()

    print(f"== Sanitizer: source={args.source_ref} out={args.out} ==")

    # Computed unconditionally (not just under --push) - also needed to
    # stamp a concrete image version into the Codespaces devcontainer.json
    # templates below, so a devcontainer.json ends up referencing the exact
    # image this same publish produces (see apply_demo_image_version),
    # instead of `:latest`, which the demo-image.yml GitHub Actions
    # workflow only ever pushes one tag of - a Codespace (or Docker itself)
    # caching an older `:latest` layer has no way to tell it's stale.
    # Falls back to "HEAD" resolving to a real commit sha regardless of the
    # working tree's dirty state, matching git_publish's own use below.
    source_sha = subprocess.run(
        ["git", "rev-parse", args.source_ref], cwd=REPO_ROOT,
        capture_output=True, text=True, check=True,
    ).stdout.strip()
    image_version = args.pipeline_version or source_sha[:12]
    ...
    #
    # demo/grafana/dashboards/ is excluded for a different but related
    # reason, confirmed live (2026-09-24): it's committed, pre-sanitized
    # output from demo/tools/build_demo_dashboards.py, which already runs
    # this exact same sanitizer (a separate `sanitize.py --dry-run`
    # subprocess) against grafanaDashboards/dashboards/ to produce it.
    # long_digit_id's regex matches ANY 21-digit run, real or already a
    # decoy from that earlier pass - re-redacting here hashes an
    # already-hashed value a second time (confirmed: a real Google Maps ID
    # in grafanaDashboards/build_ha_dashboard.py hashes to one decoy in
    # this committed dashboard JSON, but hashes AGAIN, to a different
    # decoy, if this second pass isn't skipped), permanently diverging from
    # the same real value hashed once elsewhere (e.g.
    # demo/tools/generate_demo_entities.py's own entity_id) - the exact
    # cause of the public demo's Family Map panel silently dropping Homer
    # and Marge ("no results found"), traced from the live public mirror's
    # own committed dashboard JSON.
    redactable = [
        f for f in files
        if "publish" not in f.relative_to(args.out).parts
        and f.relative_to(args.out).parts[:3] != ("demo", "grafana", "dashboards")
    ]
    ...
    print("-- Demo image version stamping --")
    stamped = apply_demo_image_version(args.out, image_version)
    print(f"   {image_version}: {stamped} placeholder(s) replaced")
    ...
    if args.push:
        ...
        if not pat:
            ...
            return 1
        git_publish(args.out, args.github_url, pat, source_sha, image_version)
