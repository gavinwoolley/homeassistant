...
def stat(title, expr, gridPos, mappings=None, thresholds=None, unit="none",
         colormode="background", text="value_and_name", legend="{{friendly_name}}", valueSize=None):
    options = {"colorMode": colormode, "graphMode": "none", "justifyMode": "auto",
               "orientation": "auto", "textMode": text,
               "reduceOptions": {"calcs": ["lastNotNull"], "fields": "", "values": False}}
    if valueSize:
        # Grafana auto-sizes a stat's number to fill the panel by default,
        # which balloons on a tall row (e.g. matched to a neighbouring
        # dyntext panel) - pin it explicitly instead of fighting height.
        options["text"] = {"valueSize": valueSize}
    return {
...
# HA/inf/c/dyntext moved up here (were originally defined further down, after
# the Status row) because the redesigned Status row below now needs them.
...
def dyntext(title, content, targets, gridPos, default_content="<em>no image</em>", styles="",
            render_mode="everyRow"):
    return {"datasource": HA, "type": "marcusolsson-dynamictext-panel", "id": nid(),
            "title": title, "gridPos": gridPos,
            "options": {"content": content, "defaultContent": default_content,
                        "editor": {"format": "auto", "language": "html"},
                        "renderMode": render_mode, "wrap": True, "contentPartials": [],
                        "helpers": "", "styles": styles, "externalStyles": []},
            "targets": targets}

# Each renderMode:"everyRow" repeat is wrapped by the plugin itself in its own
# `<div data-testid="text content">` (confirmed from the same downloaded
# plugin source used to verify everyRow itself) with a default block-level
# display plus a border-bottom "list item" look - fine for a text list, not
# for a row of avatar/pill cards. This (the panel's "Styles" option, CSS
# applied once, not repeated per row) overrides that to lay the repeats out
# left-to-right and drop the divider.
# Root-caused live against the actual plugin source (downloaded + unpacked
# module.js.map, business-text v6.1.0): TextPanel.tsx applies the panel's
# "Styles" option directly to the DIV THAT WRAPS EVERY ROW (styles.root =
# `display:flex;flex-direction:column` in TextPanel.styles.ts) - a child's
# own `display` never overrides its flex *parent's* flex-direction, which is
# why an earlier attempt (styling the row divs themselves) had no effect.
# Confirmed live on the real dashboard (Grafana MCP update_dashboard +
# get_panel_image): bare top-level declarations here DO land on that wrapper
# (flex-direction/flex-wrap below), but a nested attribute selector quoting a
# space-containing value - `[data-testid="text content"] { ... }`, with or
# without an explicit `&` - silently compiles to dead CSS (verified with a
# red-background canary that never rendered); some step in the
# options.styles -> replaceVariables -> Emotion `css` pipeline mangles the
# quoted attribute value. Swapping to a plain child combinator (`& > div`,
# no attributes/quotes) works reliably - confirmed removes the leftover
# per-row border with the same canary technique.
_ROW_FLOW_CSS = (
    'flex-direction: row !important; flex-wrap: wrap !important; '
    'align-items: flex-start !important; gap: 4px; '
    '& > div { border: none !important; margin: 0 !important; padding: 4px !important; }'
)
...
# ---- Family map: markers + trails, shared by the compact Status-row map and
# the full "Where is everyone" panel further down. Moved up here (was defined
# down by People & Presence) so the Status row can use it too. Entity is the
# most reliable GPS source for each: google_maps / phone GPS / dog collar
# always carry coords; person.bart_simpson has no dedicated GPS device so Bart Simpson only
# appears when away (the person's source switches to a GPS tracker) - and,
# for the same reason, has no trail below (no single device_tracker to pull
# /history/period for). Colours are the higher-contrast palette from an
# earlier hand-edit of the dashboard JSON (plain "red"/"green" blended into
# the OSM basemap's road/parkland hues) - ported here since this generator
# never had it; picking the old plain names again would silently reintroduce
# that bug.
PEOPLE_MAP = [
    ("A", "device_tracker.google_maps_104199578859638696260", "Homer Simpson",   "#2979FF"),
    ("B", "device_tracker.google_maps_146646711938998732988", "Marge Simpson",   "#FF1493"),
    ("C", "device_tracker.pixel_6_pro_lisa_simpson",               "Lisa Simpson", "#AA00FF"),
    ("D", "person.bart_simpson",                                       "Bart Simpson",    "#00E5FF"),
    ("E", "device_tracker.eleqqixh_dog",                   "Dog", "#00BFA5"),
]
...
def _person_trail_target(refId, eid):
    # HA's /history/period endpoint, one GET param per entry (Infinity's
    # url_options.params form) - filter_entity_id scopes it to this one
    # tracker, end_time closes the dashboard's own time range. root_selector
    # "$[0]" unwraps history's outer per-entity array since we only ever ask
    # for one entity at a time.
    return {"datasource": HA, "refId": refId, "type": "json", "source": "url",
            "format": "table", "parser": "backend",
            "url": "/history/period/${__from:date:iso}",
            "url_options": {"method": "GET", "params": [
                {"key": "filter_entity_id", "value": eid},
                {"key": "end_time", "value": "${__to:date:iso}"}]},
            "root_selector": "$[0]",
            "columns": [c("last_changed", "time", "timestamp"),
                        c("attributes.latitude", "latitude", "number"),
                        c("attributes.longitude", "longitude", "number")],
            "filters": []}

def _person_trail_layer(refId, name, color):
    return {"type": "route", "name": f"{name} trail",
            "location": {"mode": "coords", "latitude": "latitude", "longitude": "longitude"},
            "filterData": {"id": "byRefId", "options": refId},
            "config": {"style": {"color": {"fixed": color}, "opacity": 0.75,
                                  "lineWidth": 3, "arrow": 1}},
            "tooltip": False}

def _map_layers_targets(people):
    layers = [_person_layer(r, n, col) for r, _e, n, col in people]
    targets = [_person_target(r, e) for r, e, _n, _col in people]
    for r, e, n, col in people:
        if n == "Bart Simpson":
            continue
        tr = f"T{r}"
        layers.append(_person_trail_layer(tr, n, col))
        targets.append(_person_trail_target(tr, e))
    return layers, targets

def family_map(title, gridPos):
    layers, targets = _map_layers_targets(PEOPLE_MAP)
    return {
        "datasource": HA, "type": "geomap", "id": nid(), "title": title, "gridPos": gridPos,
        ...
        "description": "Markers = live position. Trail lines = movement over the dashboard time range, from HA state history.",
        "targets": targets,
    }

y = 0  # Status goes first now (per feedback) - Stats/Topology (still
# collapsed) get appended right after its block ends instead of occupying
# y=0/1 at the very top (see stats_topology_y further down, where their
# panel lists actually exist).
# ---------------- Status - a Grafana echo of the Home Assistant home page:
# ops strip, clock + weather, who's home (with photo), live motion, and
# whatever's switched on right now. This is the one row left expanded;
# everything else on the dashboard is collapsed (see collapse_rows() below).
panels.append(row("Status", y)); y += 1

# -- clock + weather, big (per feedback - was sized to match the ops strip
# tiles, now the visual anchor of the row). sensor.time/custom_date/
# outside_condition are template text sensors - no Mimir metric, so
# Infinity+dyntext. Time+Date share one panel/query (per feedback).
#
# Two dead ends before this worked, both confirmed live via Grafana MCP
# update_dashboard + get_panel_image against the real dashboard rather than
# guessed:
#   1. A JSONata object-literal root_selector (`[{"time": $[entity_id=...]
#      .state, ...}]`) building one synthesized row - the bracket-predicate
#      `$[cond]` form silently returned nothing in this Infinity datasource's
#      Go "backend" parser (everything else in this file uses the
#      `$filter($, function($v){...})` form, which does work - that's not a
#      style choice, it's the only form confirmed to evaluate here).
#   2. Fields named {{time}}/{{date}} on an everyRow row - "date" is a
#      registered Handlebars helper in this plugin (moment-based date
#      formatting), so a bare context field of the same name gets shadowed
#      by the helper instead of read (it happened to render a plausible-
#      looking date, coincidentally correct only because "today" was the
#      right answer) - "time" has no such helper and rendered blank.
# What actually works: renderMode "allRows" (not "everyRow") with a
# $filter()-based root_selector matching both entities (2 rows), and a
# Handlebars {{#each data}} loop selecting on entity_id via the plugin's
# built-in `eq` helper. Row order out of $filter isn't guaranteed, so each
# branch sets its own CSS `order` to pin Time above Date regardless.
_clock_css = (
    '<div style="display:flex;flex-direction:column;align-items:center;'
    'justify-content:center;height:100%">'
    '{{#each data}}'
    '{{#if (eq entity_id "sensor.time")}}'
    '<div style="order:1;font-size:72px;font-weight:700">{{state}}</div>'
    '{{/if}}'
    '{{#if (eq entity_id "sensor.custom_date")}}'
    '<div style="order:2;font-size:22px;color:#aaaaaa;margin-top:4px">{{state}}</div>'
    '{{/if}}'
    '{{/each}}'
    '</div>'
)
# y0 anchors this whole block: Time/Weather/Who's Home/Family Map share one
# row (4/4/8/8 of 24 cols); Motion Right Now + Switched On Right Now share
# the row below, half-width each; the ops-strip stats sit at the bottom.
y0 = y
panels.append(dyntext("Time", _clock_css,
    [inf('$filter($, function($v){ $v.entity_id = "sensor.time" or '
         '$v.entity_id = "sensor.custom_date" })',
         [c("entity_id", "entity_id"), c("state", "state")])],
    {"h": 8, "w": 4, "x": 0, "y": y0}, render_mode="allRows"))

# -- weather: same allRows/each/eq technique as Time+Date above (see that
# comment for why - two different HA entities merged into one panel/query).
# Reads sensor.outside_temperature's own state directly over Infinity rather
# than the Mimir metric the old separate "Outside Temp" stat used - same
# underlying entity, one fewer datasource for this one panel, and it means
# this whole tile lives or dies with the Infinity/HA API query like its
# neighbour instead of also depending on Mimir being scraped.
_weather_css = (
    '<div style="display:flex;flex-direction:column;align-items:center;'
    'justify-content:center;height:100%">'
    '{{#each data}}'
    '{{#if (eq entity_id "sensor.outside_temperature")}}'
    '<div style="order:1;font-size:64px;font-weight:700">'
    '{{state}}<span style="font-size:28px">&#8451;</span></div>'
    '{{/if}}'
    '{{#if (eq entity_id "sensor.outside_condition")}}'
    '<div style="order:2;font-size:22px;color:#aaaaaa;text-transform:capitalize;'
    'margin-top:4px">{{state}}</div>'
    '{{/if}}'
    '{{/each}}'
    '</div>'
)
panels.append(dyntext("Weather", _weather_css,
    [inf('$filter($, function($v){ $v.entity_id = "sensor.outside_temperature" or '
         '$v.entity_id = "sensor.outside_condition" })',
         [c("entity_id", "entity_id"), c("state", "state")])],
    {"h": 8, "w": 4, "x": 4, "y": y0}, render_mode="allRows"))

# -- who's home: one repeated card per person (renderMode "everyRow" reruns
# the same Handlebars template once per matched row, so a single panel +
# single multi-row query gives one card per person - see mermaid-validate-
# style verification note in the PR: confirmed against the actual installed
# plugin (Business Text / marcusolsson-dynamictext-panel 6.x) source, the
# EVERY_ROW branch does exactly `rows.map(row => render(row, content))`, so
# each row's columns are plain top-level template variables ({{state}}, not
# {{data.state}} or an {{#each}} loop). Photo comes from the person entity's
# own attributes.entity_picture (verified live - all four have it set,
# matching the /local/*.jpg the Lovelace picture-entity cards hardcode).
# _ROW_FLOW_CSS (panel "Styles", not per-row content) makes the repeats sit
# left-to-right instead of stacking with a scrollbar - see its own comment
# for the root cause (it's the shared *panel-level* fix for this plugin).
# Avatar size shrunk from an earlier 130px/64px card (verified live: at this
# panel's actual on-dashboard width - a third of the row, sharing space with
# Family Map - only 2 of 5 people fit per line at that size and Dog wrapped
# off the bottom, invisible). 60px/36px fits all 5 on one row.
_family_css = (
    '<style>.hs-home{border-color:#2ecc71 !important}'
    '.hs-not_home{border-color:#e74c3c !important}</style>'
    '<div style="display:flex;flex-direction:column;align-items:center;'
    'width:76px;margin:2px">'
    '<img class="hs-{{state}}" src="https://home.example.com:8123{{image}}" '
    'style="width:52px;height:52px;border-radius:50%;object-fit:cover;border:3px solid #888" />'
    '<div style="margin-top:4px;font-size:12px;color:#dddddd;text-align:center;line-height:1.1">{{name}}</div>'
    '<div style="font-size:10px;text-transform:capitalize;color:#999999">{{state}}</div>'
    '</div>'
)
_family_selector = (
    '$filter($, function($v){ $v.entity_id in '
    '["person.homer_simpson","person.marge_simpson","person.lisa_simpson","person.bart_simpson",'
    '"device_tracker.eleqqixh_dog"] })'
)
panels.append(dyntext("Who's Home", _family_css,
    [inf(_family_selector, [c("attributes.friendly_name", "name"), c("state", "state"),
                            c("attributes.entity_picture", "image")])],
    {"h": 8, "w": 8, "x": 8, "y": y0}, styles=_ROW_FLOW_CSS))

# -- family map: same markers+trails as the full "Where is everyone" panel
# further down (see family_map()), just smaller and living beside Who's Home
# instead of its own full-width row.
panels.append(family_map("Family Map", {"h": 8, "w": 8, "x": 16, "y": y0}))

# -- live motion (repeated pill per triggered sensor; empty when clear) and
# switched-on-right-now now share a row, half-width each (per feedback -
# motion used to be up in the Time/Weather/WhoHome row, switched-on used to
# be its own full-width row below).
panels.append(dyntext("Motion Right Now",
    '<span style="display:inline-block;margin:3px;padding:5px 12px;border-radius:12px;'
    'background:#e67e22;color:#000000;font-size:12px;font-weight:600">{{name}}</span>',
    [inf('$filter($, function($v){ ($substringBefore($v.entity_id,".")="binary_sensor") and '
         '($contains($v.entity_id,"motion") or $contains($v.entity_id,"occupancy")) and '
         '$v.state="on" })',
         [c("attributes.friendly_name", "name")])],
    {"h": 6, "w": 12, "x": 0, "y": y0 + 8}, default_content='<span style="color:#666">No motion detected</span>',
    styles=_ROW_FLOW_CSS))

# -- switched on right now: the curated SW/LIGHT lists (same "physical
# devices only" set the collapsed Lighting & Switches row uses below -
# camera PTZ, motion-automation-enable toggles, Wi-Fi blocks deliberately
# excluded), not "every switch.* domain entity". Filtered server-side to
# state="on", so an off device just never produces a row/card.
_active_ids = [f'"switch.{e}"' for e in SW] + [f'"light.{e}"' for e in LIGHT]
_active_selector = ('$filter($, function($v){ $v.entity_id in [' + ",".join(_active_ids) +
                    '] and $v.state="on" })')
panels.append(dyntext("Switched On Right Now",
    '<span style="display:inline-block;margin:3px;padding:5px 12px;border-radius:12px;'
    'background:#2ecc71;color:#000000;font-size:12px;font-weight:600">{{name}}</span>',
    [inf(_active_selector, [c("attributes.friendly_name", "name")])],
    {"h": 6, "w": 12, "x": 12, "y": y0 + 8},
    default_content='<span style="color:#666">Nothing switched on</span>', styles=_ROW_FLOW_CSS))

# -- ops strip: just the two things worth a glance at a distance, moved to
# the bottom of the row per feedback (was at the top). (Entities Online /
# Automations Fired dropped earlier - noise for this row, not gone from the
# dashboard, they've never existed anywhere else so there's nothing to
# remove elsewhere.)
panels.append(stat("Active Alerts",
                   'homeassistant_sensor_state{entity="sensor.active_alerts"} or vector(0)',
                   {"h": 5, "w": 12, "x": 0, "y": y0 + 14}, unit="none", text="value", legend="Firing",
                   colormode="value",
...
panels.append(stat("Updates Available",
                   ...
                   {"h": 5, "w": 12, "x": 12, "y": y0 + 14}, unit="none", text="value", legend="Updates",
                   colormode="value",
                   thresholds=[{"color": "text", "value": None}, {"color": "yellow", "value": 1},
                               {"color": "orange", "value": 10}]))
y = y0 + 19

# stats_topology_y: Stats + Topology (both still collapsed) go right after
# the Status block instead of at y=0/1 above it - Status is the first,
# expanded row now (per feedback). Reserve their 2 rows of y-space here;
# the crow() calls themselves are further down, once _STATS_PANELS and
# _DIAGRAM_PANELS exist.
stats_topology_y = y
y += 2
...
# Two collapsed rows right after Status (per feedback - Status is first/
# expanded now, these used to sit above it at y=0/1): infra stats first,
# then the diagram. Array position doesn't affect layout (gridPos does),
# so appending here - after their panel lists exist - is fine.
panels.append(crow("Stats", stats_topology_y, _STATS_PANELS, collapsed=True))
panels.append(crow("Topology", stats_topology_y + 1, _DIAGRAM_PANELS, collapsed=True))
...
# ---- People & Presence (PEOPLE_MAP + map helpers now live up near the top
# of the file, next to SW/LIGHT - the Status row's compact map needs them
# too; see family_map() there for the full comment.)
_geo = family_map("Where is everyone", {"h": 11, "w": 14, "x": 0, "y": 0})
...
_mow_kv = ha_kv("Status", [
    ...
    # default=0 on as_timestamp - confirmed live (2026-09-22): without it,
    # this whole panel hard-errors (HTTP 400, "as_timestamp got invalid
    # input 'unknown' ... no default was specified") the moment this one
    # sensor is ever unavailable, not just empty - not a demo-only gap
    # (that sensor genuinely doesn't exist in the demo's generated
    # entities at all), production's own mower could hit the exact same
    # error if its connect-expiration sensor is ever transiently
    # unavailable. Degrades to epoch/"01 Jan 1970" instead of crashing.
    ("Connect expires", f"as_timestamp(states('sensor.{M355}_connect_expiration'), 0) | timestamp_custom('%d %b %Y', true)"),
], {"h": 9, "w": 10, "x": 14, "y": 5})
...
# Collapse every still-expanded row except Status (the redesigned home-page-
# style row above) - operates on gridPos.y bands, not array position, so it
# doesn't matter that some panels above were appended out of visual order.
# Already-collapsed rows (built with crow()) are untouched: their members
# live nested in row["panels"] already, never as separate top-level entries,
# so there's nothing for this to see or re-bucket.
def collapse_open_rows(panels, keep_open=("Status",)):
    row_ys = sorted(p["gridPos"]["y"] for p in panels if p["type"] == "row")

    def section_end(y0):
        later = [ry for ry in row_ys if ry > y0]
        return min(later) if later else None

    consumed = set()
    for p in panels:
        if p["type"] == "row" and not p.get("collapsed") and p["title"] not in keep_open:
            y0 = p["gridPos"]["y"]
            y1 = section_end(y0)
            children = [q for q in panels if q["type"] != "row"
                        and q["gridPos"]["y"] > y0
                        and (y1 is None or q["gridPos"]["y"] < y1)]
            for ch in children:
                consumed.add(id(ch))
            p["collapsed"] = True
            p["panels"] = children
    return [p for p in panels if id(p) not in consumed]

panels = collapse_open_rows(panels)
