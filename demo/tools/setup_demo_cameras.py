...
# parents[1], not parents[2]+"demo" - this script's own directory
# (demo/tools/) has a different parent depending on how it's invoked: run
# from a full repo checkout (`python demo/tools/setup_demo_cameras.py`),
# parents[1] is the repo's demo/ directory directly. Run inside the demo
# container via the setup_demo_cameras shell_command (generate_demo_entities.py's
# setup_demo_cameras_automation), it's /config/tools/setup_demo_cameras.py -
# /config IS demo/'s bind-mounted contents (docker-compose.demo.yml's
# `volumes: - ./:/config`), not a repo root with a nested demo/ under it, so
# parents[2] overshot past /config to the container's filesystem root.
# Confirmed live (2026-09-23): every automated run 404'd with
# FileNotFoundError: [Errno 2] No such file or directory: '/demo/generated/cameras.json'
# - manual/local runs never hit this since a real repo checkout happens to
# have a "demo" directory sitting at that same depth by coincidence.
DEMO_DIR = Path(__file__).resolve().parents[1]
CAMERA_MAP_FILE = DEMO_DIR / "generated" / "cameras.json"
WWW_DIR = DEMO_DIR / "www"
CUSTOMIZE_FILE = DEMO_DIR / "customize.yaml"
...
async def remove_stale_camera_entries(session: aiohttp.ClientSession, base_url: str, access_token: str, cameras: dict) -> None:
    """local_file's config flow dedups by file_path, not by name/entity_id -
    a camera object_id rename (generate_demo_entities.py's OBJECT_ID_SANITIZE)
    doesn't touch the underlying static image file, so the OLD entry (its own
    entity_id never itself renamed - config-flow entities don't follow a
    config change the way declarative YAML ones do) silently blocks the
    config flow from ever creating a new entry under the new name: it just
    reports "already_configured" for the wanted file_path, while the
    dashboard's new entity_id reference sits at "Entity not found" forever.
    Confirmed live (2026-09-23) after the Homer Simpson/Dog->Simpsons rename:
    camera.bart_simpson_room_last_motion (old entity_id, never renamed) still served
    /config/www/bart_room_last_motion.jpg, the same file the new "Bart
    Simpson Room Last Motion" camera also wants, so the new one could never
    be created no matter how many times this script ran.

    local_file camera entities expose their own file_path as a state
    attribute (confirmed live), so this finds any camera already serving a
    file_path this run wants under a *different* entity_id than the one
    that file_path's current object_id would produce, and removes its whole
    config entry (which cleanly removes the stale entity too) so the
    create_entry attempt below succeeds instead of skipping. Entity registry
    lookups are WS-only, same as apply_customize_registry_names above.
    """
    wanted_object_id = {cam["file_path"]: object_id for object_id, cam in cameras.items()}
    async with session.get(f"{base_url}/api/states", headers={"Authorization": f"Bearer {access_token}"}) as resp:
        states = await resp.json()
    stale = [
        s for s in states
        if s["entity_id"].startswith("camera.")
        and s["attributes"].get("file_path") in wanted_object_id
        and s["entity_id"] != f"camera.{wanted_object_id[s['attributes']['file_path']]}"
    ]
    if not stale:
        return
    ws_url = base_url.replace("https://", "wss://", 1).replace("http://", "ws://", 1) + "/api/websocket"
    async with session.ws_connect(ws_url, ssl=False) as ws:
        ...
        if (await ws.receive_json()).get("type") != "auth_ok":
            print("stale camera cleanup: websocket auth failed")
            return
        msg_id = 1
        for s in stale:
            await ws.send_json({"id": msg_id, "type": "config/entity_registry/get", "entity_id": s["entity_id"]})
            reg = await ws.receive_json()
            entry_id = (reg.get("result") or {}).get("config_entry_id")
            msg_id += 1
            if not entry_id:
                print(f"stale camera cleanup: {s['entity_id']} has no config_entry_id, skipping")
                continue
            async with session.delete(
                f"{base_url}/api/config/config_entries/entry/{entry_id}",
                headers={"Authorization": f"Bearer {access_token}"},
            ) as resp:
                if resp.status == 200:
                    print(f"stale camera cleanup: removed {s['entity_id']} (file_path now free)")
                else:
                    print(f"stale camera cleanup: failed to remove {s['entity_id']} - {resp.status} {await resp.text()}")
...
async def main() -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await remove_stale_camera_entries(session, args.base_url, access_token, cameras)
